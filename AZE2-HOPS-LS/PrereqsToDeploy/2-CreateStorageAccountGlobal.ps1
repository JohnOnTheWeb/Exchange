...
$saNameDiag = 'sanmyappglobaldiageus2'

# create storage accounts
New-AzureRmStorageAccount -ResourceGroupName $rgName -Name $saname -SkuName Standard_LRS -Location $location -Kind Storage `
                                -EnableEncryptionService "Blob,File" -EnableHttpsTrafficOnly 

New-AzureRmStorageAccount -ResourceGroupName $rgName -Name $sanameDiag -SkuName Standard_LRS -Location $location -Kind Storage `
                                -EnableEncryptionService "Blob,File" -EnableHttpsTrafficOnly 
